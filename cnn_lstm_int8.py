...
def representative_dataset():
    for i in range(100):
      yield [np.expand_dims(np.test_data[i].astype(np.float32),axis=0),np.expand_dims(data_lstm_test[i].astype(np.float32),axis=0),np.expand_dims(test_w[i].astype(np.float32),axis=0),np.expand_dims(test_d[i].astype(np.float32),axis=0)]
...
# Save the model.
with open('cnn_lstm_int8_solid.tflite', 'wb') as f:
...
interpret_tflite('cnn_lstm_qint16_solid.tflite')
interpret_tflite_by_sample('cnn_lstm_qint16_solid.tflite')
# -*- coding: utf-8 -*-
"""cnn_lstm_lovish_TA.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/12UATd1XpM7vzkYNYvvsqP0Dk9xLDgju0
"""
...
[test_data,data_lstm_test,test_w,test_d]
print("test_data.shape  = ",test_data.shape)
print("data_lstm_test.shape  = ",data_lstm_test.shape)
print("test_w.shape  = ",test_w.shape)
print("test_d.shape  = ",test_d.shape)

def inverse_scale(test_predict):
  return scaler.inverse_transform(test_predict[:, -1])

data_lstm_train = data_lstm

train_data.shape,data_lstm_train.shape,train_w.shape,train_d.shape, train_l.shape

finetuning_ratio = 0.4
train_data_subset = train_data[:int(finetuning_ratio*len(train_l))]  
data_lstm_train_subset = data_lstm_train[:int(finetuning_ratio*len(train_l))]  
train_w_subset = train_w[:int(finetuning_ratio*len(train_l))]  
train_d_subset = train_d[:int(finetuning_ratio*len(train_l))]  
train_l_subset = train_l[:int(finetuning_ratio*len(train_l))]

train_data_subset.shape,data_lstm_train_subset.shape,train_w_subset.shape,train_d_subset.shape, train_l_subset.shape
...
his = quant_aware_model.fit([train_data_subset,data_lstm_train_subset,train_w_subset,train_d_subset],train_l_subset,epochs=50,validation_split=0.3)
...
test_predict=model.predict([test_data,data_lstm_test,test_w,test_d])
test_predict_quant = quant_aware_model.predict([test_data,data_lstm_test,test_w,test_d])

predicted = inverse_scale(test_predict).flatten()
predicted_quant = inverse_scale(test_predict_quant).flatten()

predicted.flatten().shape, test_l.shape, predicted_quant.flatten().shape

print ("MAE:", MAE(predicted, test_l))
print ("MAPE:", MAPE(predicted, test_l))
print ("RMSE:", RMSE(predicted, test_l))

print ("(Quant) MAE:", MAE(predicted_quant, test_l))
print ("(Quant) MAPE:", MAPE(predicted_quant, test_l))
print ("(Quant) RMSE:", RMSE(predicted_quant, test_l))
